/*
    DiepCustom - custom tank game server that shares diep.ios WebSocket protocol
...
class InventoryHandler {
    static setInventory(view) {
        ...
        const length = view[2];
        for(let i = 0; i < length; ++i) InventoryHandler.createSlotView(i);
    }

    static setSlot(view) {
        const idx = view[1];
        const data = JSON.parse(new TextDecoder().decode(view.slice(2, view.length - 1))); // { e: isEmpty, r: ratity, n: name, a: animationstate }
        InventoryHandler.updateSlotView(idx, data.e, data.n, data.r, data.a);
    }

    static useItem(idx) {
        return Game.socket.send(new Uint8Array([30, idx, 1]));
    }

    static dropItem(idx) {
        return Game.socket.send(new Uint8Array([30, idx, 0]));
    }

    static updateSlotView(idx, isEmpty, name, rarity, animationState) {
        const itemEl = document.getElementsByClassName("item")[idx];
        const imgEl = document.getElementsByClassName("item-img")[idx];
        const nameEl = document.getElementsByClassName("item-name")[idx];

        if(isEmpty) {
            const _first_header = itemEl.firstChild;
            const _first_name = nameEl.firstChild;
            itemEl.insertBefore(document.createTextNode(`Slot ${idx + 1}`), itemEl.firstChild)
            nameEl.insertBefore(document.createTextNode("Empty"), nameEl.firstChild);
            itemEl.removeChild(_first_header);
            nameEl.removeChild(_first_name);
            imgEl.src = ITEM_ICONS.empty;
            nameEl.style.color = "black";
        } else {
            const _first_header = itemEl.firstChild;
            const _first_name = nameEl.firstChild;
            itemEl.insertBefore(document.createTextNode(`Slot ${idx + 1} (${(SLOT_KEYBINDS[idx] || "none").toUpperCase()})`), itemEl.firstChild);
            nameEl.insertBefore(document.createTextNode(name.toString()), nameEl.firstChild);
            itemEl.removeChild(_first_header);
            nameEl.removeChild(_first_name);
            imgEl.src = ITEM_ICONS[rarity].toString();
            nameEl.style.color = ITEM_COLOR[rarity].toString();
            if(animationState) {
                imgEl.style.animation = `fade ${animationState}s`;
            }
            else {
    ...
    static createSlotView(idx) {
        ...
        imgEl.src = ITEM_ICONS.empty;
        itemEl.appendChild(document.createTextNode(`Slot ${idx + 1}`));
        nameEl.appendChild(document.createTextNode("Empty"));
...
class ASMConsts {
    ...
    static contextFillText(ctxId, text) {
        const _text = Module.UTF8ToString(text);
        const ctx = Module.cp5.contexts[ctxId];
        const _fill = ctx.fillStyle;
        switch(_text) {
            case "Common Item":
                ctx.fillStyle = ITEM_COLOR.common;
                break;
            case "Rare Item":
                ctx.fillStyle = ITEM_COLOR.rare;
                break;
            case "Epic Item":
                ctx.fillStyle = ITEM_COLOR.epic;
                break;
        }
    ...
    static createWebSocket(url) {
        ...
        ws.onmessage = function(e) {
            ...
            switch(view[0]) {
                ...
                case 30: {
                    return InventoryHandler.setInventory(view);
                }
                case 31: {
