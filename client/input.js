...
window.setupInput = () => {
    ...
    window.onkeydown = e => {
        const idx = SLOT_KEYBINDS.findIndex(key => key === e.key);
        const idx2 = SLOT_KEYBINDS.findIndex(key => key.toUpperCase() === e.key);
        if(idx !== -1) InventoryHandler.useItem(idx);
        else if(idx2 !== -1) InventoryHandler.dropItem(idx2);
        window.input.flushInputHooks();
