...
export default class Client {
    ...
    public inventory: InventoryManager = new InventoryManager(this);

    /** Item effects */
    public invisibilityEffect: number = 0;
    public damageEffect: number = 0;
    public healthEffect: number = 0;
    public reloadEffect: number = 0;
    public fovEffect: number = 0;
    public invulnerabilityEffect: number = 0;
    public spawnBase: TeamBase | null = null;
    public absorptionEffect: number = 0;
    public extraStatEffect: number = 0;
    ...
    public constructor(game: GameServer, ws: WebSocket, ipAddress: string) {
        ...
        this.droneSpawner = new DroneSpawner(this.game);
    }
    ...
    private handleIncoming(header: number, data: Uint8Array): void {
        ...
        switch (header) {
            ...
            case ServerBound.Spawn: {
                ...
                this.inventory.sendInventoryUpdate();

                camera.cameraData.statsAvailable += this.extraStatEffect;
                return;
            }
            ...
            case ServerBound.TCPInit:
                ...
                return;
            case ServerBound.ItemAction:
                const slot = r.u8();
                const isUse = r.u8();
                if(isUse === 0) return this.inventory.dropItem(slot);
                if(isUse === 1) return this.inventory.useItem(slot);
            default:
