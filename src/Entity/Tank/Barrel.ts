...
export class ShootCycle {
    ...
    public tick() {
        ...
        if((this.barrelEntity.tank.cameraEntity as ClientCamera)?.client?.reloadEffect) {
            (this.barrelEntity.tank.cameraEntity as ClientCamera).client.reloadEffect--;
            reloadTime /= 2;
        }
...
export default class Barrel extends ObjectEntity {
    ...
    public shoot() {
        ...
        const scatterAngle = (Math.PI / 180) * (this.definition.bullet.scatterRate * (1 - ((this.tank.cameraEntity as ClientCamera)?.client?.accuracyEffect || 0))) * (Math.random() - .5) * 10;
